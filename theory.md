...
```
В С++ глубоке копіювання об'єкту має здійснюватись по дефолту оператором=, на відміну від інших мов, де об'єкт це референс на нього і копіювання об'єкту виконується поверхнево через оператор=, тому в С++ для реалізації патерна не потрібно робити метод `clone()` (як це робиться в класичному прикладі імплементації патерна).
